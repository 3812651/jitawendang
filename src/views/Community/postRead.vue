<template>
  <div class="container">
    <div class="postRead">
      <div class="main-area article-area shadow">
        <article class="article">
          <div class="author-info-block">
            <div class="lazy avatar avatar loaded" :style="{backgroundImage:'url('+avatar+')'}"></div>
            <div class="author-info-box">
              <div class="username username ellipsis">
                <span class="name" style="max-width: calc(100% - 50px)">李小龙</span>
              </div>

              <div class="meta-box">
                <span>2020年11月12号</span>
                <span class="views-count">阅读</span>
                <span>666</span>
              </div>
            </div>
            <Button type="success" size="small" @click="follow" v-if="isfollow">已关注</Button>
            <Button type="warning" ghost size="small" @click="follow" v-else>关注</Button>

          </div>
        </article>
      </div>
      <div class="sidebar">
        <div class="sidebar-block author-block shadow">
          <div class="block-title">关于作者
            <Icon color="#f90" type="md-ribbon" />
          </div>
          <div class="block-body">
            <a href="" class="user-item" target="_blank">
              <div class="lazy avatar avatar loaded" :style="{backgroundImage:'url('+avatar+')'}"></div>
              <div class="info-box">
                <a class="username"><span class="name">李小龙</span> </a>
                <p class="desc">哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈</p>
              </div>
            </a>
            <div class="stat-item item getZanbox">
              <svg xmlns="http://www.w3.org/2000/svg" width="25" height="26" viewBox="0 0 25 26" class="icon">
                <g fill="none" fill-rule="evenodd" transform="translate(0 .57)">
                  <ellipse cx="12.5" cy="12.57" fill="#f5eddb" rx="12.5" ry="12.57"></ellipse>
                  <path fill="#ff9900"
                    d="M8.596 11.238V19H7.033C6.463 19 6 18.465 6 17.807v-5.282c0-.685.483-1.287 1.033-1.287h1.563zm4.275-4.156A1.284 1.284 0 0 1 14.156 6c.885.016 1.412.722 1.595 1.07.334.638.343 1.687.114 2.361-.207.61-.687 1.412-.687 1.412h3.596c.38 0 .733.178.969.488.239.317.318.728.21 1.102l-1.628 5.645a1.245 1.245 0 0 1-1.192.922h-7.068v-7.889c1.624-.336 2.623-2.866 2.806-4.029z">
                  </path>
                </g>
              </svg>
              <span class="content">获得点赞<span class="count">438</span></span>
            </div>
            <div class="stat-item item">
              <svg width="25" height="25" viewBox="0 0 25 25" class="icon">
                <g fill="none" fill-rule="evenodd">
                  <circle cx="12.5" cy="12.5" r="12.5" fill="#f5eddb"></circle>
                  <path fill="#ff9900"
                    d="M4 12.5S6.917 7 12.75 7s8.75 5.5 8.75 5.5-2.917 5.5-8.75 5.5S4 12.5 4 12.5zm8.75 2.292c1.208 0 2.188-1.026 2.188-2.292 0-1.266-.98-2.292-2.188-2.292-1.208 0-2.188 1.026-2.188 2.292 0 1.266.98 2.292 2.188 2.292z">
                  </path>
                </g>
              </svg>
              <span class="content">文章被阅读<span class="count">6666</span></span>
            </div>
          </div>
        </div>
        <Affix :offset-top="15">
          <div class="sidebar-block catalog-block shadow">
            <div class="block-title">目录
              <Icon type="md-list" />
            </div>
            <div class="block-body">
              <ul class="catalog-list">
                <li class="item d1 active">
                  <a href="#heading-0" title="Element3开发内幕 - Vue CLI插件开发">Element3开发内幕 - Vue CLI插件开发</a>
                  <ul class="sub-list">
                    <li class="item d2"><a href="#heading-1" title="一、什么是Vue CLI插件">一、什么是Vue CLI插件</a>
                    </li>
                    <li class="item d2"><a href="#heading-2" title="二、功能实现">二、功能实现</a>
                      <ul class="sub-list">
                        <li class="item d3"><a href="#heading-3" title="1. 搭建框架">
                            1. 搭建框架
                          </a>
                          <ul class="sub-list">
                            <li class="item d4" style="display: none;"><a href="#heading-4" title="1.1 初始npm库">
                                1.1 初始npm库
                              </a>
                              <!---->
                            </li>
                          </ul>
                        </li>
                        <li class="item d3"><a href="#heading-5" title="2. 安装配置命令行交互">
                            2. 安装配置命令行交互
                          </a>
                          <!---->
                        </li>
                        <li class="item d3"><a href="#heading-6" title="3. 代码生成器Generator">
                            3. 代码生成器Generator
                          </a>
                          <ul class="sub-list">
                            <li class="item d4" style="display: none;"><a href="#heading-7" title="3.1 添加依赖">
                                3.1 添加依赖
                              </a>
                              <!---->
                            </li>
                            <li class="item d4" style="display: none;"><a href="#heading-8" title="3.2 添加插件">
                                3.2 添加插件
                              </a>
                              <!---->
                            </li>
                            <li class="item d4" style="display: none;"><a href="#heading-9" title="3.3 添加插件引用">
                                3.3 添加插件引用
                              </a>
                              <!---->
                            </li>
                            <li class="item d4" style="display: none;"><a href="#heading-10" title="3.4 添加代码示例">
                                3.4 添加代码示例
                              </a>
                              <!---->
                            </li>
                          </ul>
                        </li>
                        <li class="item d3"><a href="#heading-11" title="4. Service处理">
                            4. Service处理
                          </a>
                          <!---->
                        </li>
                      </ul>
                    </li>
                    <li class="item d2"><a href="#heading-12" title="三、本地调试"> 三、本地调试</a>
                    </li>
                  </ul>
                </li>
                <li class="item d1 ">
                  <a href="#heading-0" title="年轻人要讲武德！！！点赞、关注、收藏都要安排起来！！！">年轻人要讲武德！！！点赞、关注、收藏都要安排起来！！！</a>
                </li>
              </ul>
            </div>
          </div>
        </Affix>
      </div>
      <div class="article-suspended-panel">
        <div class="like-btn panel-btn like-adjust with-badge" badge="174"></div>
        <div class="comment-btn panel-btn comment-adjust with-badge" badge="109"></div>
        <div class="collect-btn panel-btn">
          <!---->
        </div>
        <div class="share-title">分享</div>
        <div class="weibo-btn share-btn panel-btn"></div>
        <div class="qq-btn share-btn panel-btn"></div>
        <div class="wechat-btn share-btn panel-btn">
          <img src="" class="wechat-qr-code-img shadow" style="display:none;">
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      isfollow: false,
      avatar:
        "https://mirror-gold-cdn.xitu.io/16beee0a51718efb050?imageView2/1/w/100/h/100/q/85/format/webp/interlace/1",
    };
  },
  methods: {
    follow() {
      this.isfollow = this.isfollow == false ? true : false;
    },
  },
};
</script>

<style lang="less" scoped>
@import "../../assets/css/publicVar.less";

.article-suspended-panel {
  position: fixed;
  margin-left: -0.7rem;
  top: 2.3rem;

  .share-title {
    margin: 0.25rem 0 0.1rem;
    font-size: 0.1rem;
    text-align: center;
    color: #c6c6c6;
    -webkit-user-select: none;
    -moz-user-select: none;
    -ms-user-select: none;
    user-select: none;
  }

  .panel-btn.like-btn.like-adjust {
    background-position: 53% 46%;
  }

  .panel-btn.comment-btn.comment-adjust {
    background-position: 50% 55%;
  }

  .panel-btn.like-btn {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.like-btn:hover {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.comment-btn {
    background-image: url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxNCIgaGVpZ2h0PSIxMyIgdmlld0JveD0iMCAwIDE0IDEzIj4KICAgIDxwYXRoIGZpbGw9IiNCMkJBQzIiIGZpbGwtcnVsZT0iZXZlbm9kZCIgZD0iTTE0IDB2OS4yM0g4LjgxNUwzLjM3IDEzVjkuMjNIMFYweiIvPgo8L3N2Zz4K);
  }

  .panel-btn.comment-btn:hover {
    background-image: url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxNCIgaGVpZ2h0PSIxMyIgdmlld0JveD0iMCAwIDE0IDEzIj4KICAgIDxwYXRoIGZpbGw9IiM4QTkzQTAiIGZpbGwtcnVsZT0iZXZlbm9kZCIgZD0iTTE0IDB2OS4yM0g4LjgxNUwzLjM3IDEzVjkuMjNIMFYweiIvPgo8L3N2Zz4K);
  }

  .panel-btn.collect-btn {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.collect-btn:hover {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.weibo-btn {
    background-image: url(//s3.pstatp.com/toutiao/xitu_juejin_web/img/weibo.2076a57.svg);
  }

  .panel-btn.weibo-btn:hover {
    background-image: url(//s3.pstatp.com/toutiao/xitu_juejin_web/img/weibo-hover.9abf502.svg);
  }

  .panel-btn.qq-btn {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.qq-btn:hover {
    background-image: url(data:image/svg+xml;base64,);
  }

  .panel-btn.wechat-btn {
    background-image: url(//s3.pstatp.com/toutiao/xitu_juejin_web/img/wechat.63e1ce0.svg);
  }

  .panel-btn.wechat-btn:hover {
    background-image: url(//s3.pstatp.com/toutiao/xitu_juejin_web/img/wechat-hover.c8ce019.svg);
  }

  .panel-btn {
    position: relative;
    margin-bottom: 0.075rem;
    width: 0.3rem;
    height: 0.3rem;
    background-color: #fff;
    background-position: 50%;
    background-repeat: no-repeat;
    border-radius: 50%;
    box-shadow: 0 2px 4px 0 rgba(0, 0, 0, 0.04);
    cursor: pointer;
  }

  .panel-btn.with-badge:after {
    content: attr(badge);
    position: absolute;
    top: 0;
    left: 75%;
    padding: 0.01rem 0.04rem;
    font-size: 0.1rem;
    text-align: center;
    line-height: 1;
    white-space: nowrap;
    color: #fff;
    background-color: #b2bac2;
    border-radius: 0.07rem;
    transform-origin: left top;
    transform: scale(0.75);
  }
}
.catalog-block {
  min-height: 2.5rem;
}
.block-body {
  padding: 0.13rem;
  .catalog-list {
    position: relative;
    .item {
      margin: 0;
      padding: 0;
      //   font-size: 0.1167rem;
      font-weight: 400;
      line-height: 1.3;
      list-style: none;
      a {
        color: #333;
      }
    }
    .item.active > a {
      color: #007fff;
      background-color: #ebedef;
    }
    .item a {
      display: block;
      position: relative;
      padding: 4px 0 4px 12px;
      white-space: nowrap;
      overflow: hidden;
      text-overflow: ellipsis;
      &:before {
        content: "";
        position: absolute;
        top: 50%;
        left: 7px;
        margin-top: -2px;
        width: 4px;
        height: 4px;
        background-color: currentColor;
        border-radius: 50%;
      }
      &:hover {
        background-color: #ebedef;
      }
    }
    .item.d1 {
      font-weight: 600;
      color: #000;
      & > a {
        margin: 6px 0;
        padding: 4px 0 4px 21px;
      }
      & > a:before {
        left: 7px;
        margin-top: -3px;
        width: 6px;
        height: 6px;
      }
    }
    .item.d2 > a {
      padding-left: 36px;
      &:before {
        left: 24px;
      }
    }
    .item.d3 > a {
      padding-left: 51px;
      &:before {
        left: 39px;
      }
    }
  }
  .catalog-list:before {
    content: "";
    position: absolute;
    top: 0;
    left: 7px;
    bottom: 0;
    width: 2px;
    background-color: #ebedef;
    opacity: 0.5;
  }
}
.author-block {
  .getZanbox {
    margin-bottom: 0.08rem;
  }

  .stat-item {
    .content {
      font-size: 0.16rem;
      color: #000;
    }

    .count {
      margin: 0 0.35em;
      font-weight: 500;
    }

    .icon {
      margin-right: 0.2rem;
    }
  }

  .item {
    display: flex;
    align-items: center;
  }

  .user-item {
    display: flex;
    align-items: center;
    margin-bottom: 0.13rem;

    .avatar {
      flex: 0 0 auto;
      margin-right: 0.1rem;
      width: 0.6rem;
      height: 0.6rem;
      border-radius: 50%;
    }

    .desc {
      font-size: 0.12rem;
      height: 0.35rem;
      color: #72777b;
      display: -webkit-box;
      -webkit-box-orient: vertical;
      -webkit-line-clamp: 2;
      overflow: hidden;
    }
  }
}

.lazy.loaded:before {
  opacity: 0;
  pointer-events: none;
}

.lazy:not(.immediate):before {
  transition: opacity 0.2s;
}

.lazy:before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background-color: inherit;
  border-radius: inherit;
}

.username {
  width: 100%;
  font-size: 0.16rem;
  font-weight: 700;
  color: #333;

  .name {
    display: inline-block;
    vertical-align: top;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
}

.author-info-block {
  display: flex;
  align-items: center;
  margin-bottom: 0.2rem;

  .avatar {
    flex: 0 0 auto;
    margin-right: 0.1rem;
    width: 0.333rem;
    height: 0.333rem;
    border-radius: 50%;
  }

  .author-info-box {
    min-width: 0;
    flex-grow: 1;
    flex: 1;

    .meta-box {
      font-size: 0.13rem;
      color: #909090;
    }

    .views-count {
      margin: 0 0.05rem 0 0.2rem;
    }

    .ellipsis {
      white-space: nowrap;
      overflow: hidden;
      text-overflow: ellipsis;
    }
  }
}

.lazy {
  position: relative;
}

.avatar,
.avatar.xmas:after {
  background-position: 50%;
  background-repeat: no-repeat;
}

.avatar {
  display: inline-block;
  position: relative;
  background-size: cover;
  background-color: #eee;
}

.sidebar {
  position: absolute;
  top: 0;
  right: 0;
  width: 2.4rem;

  .sidebar-block:not(.pure) {
    background-color: #fff;

    .block-title {
      padding: 0.1rem 0.13rem;
      font-size: 0.16rem;
      color: #333;
      border-bottom: 1px solid hsla(0, 0%, 58.8%, 0.1);
    }
  }

  .sidebar-block {
    position: relative;
    margin-bottom: 0.15rem;
    border-radius: 2px;
  }
}

.article {
  display: block;
  margin-bottom: 0.3rem;
}

.article-area {
  margin-bottom: 0.15rem;
  padding: 0.2rem 0.2rem 0;
}

.main-area {
  position: relative;
  width: 700px;
  height: 1600px; //记得删除
  max-width: 100%;
  background-color: #fff;
  border-radius: 2px;
  box-sizing: border-box;
}

.container {
  @margin-auto();
}

.shadow {
  box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);
}
</style>

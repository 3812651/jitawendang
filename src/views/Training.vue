      ...
      <div class="card_box">
        ...
        <ul id="second_ul" v-show="checkIndex == 1">
          <li v-for="item in secondList" :key="item.iconClass" :id="item.id">
            ...
            <i class="play_btn" v-show="btn_flag1" @click="play(item.id,'btn_flag1')"></i>
            <i class="pause_btn" v-show="!btn_flag1" @click="btn_flag1 = !btn_flag1"></i>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      checkIndex: 0,
      li_index: 4,
      btn_flag: true, //first_ul暂停和播放
      btn_flag1: true, //second_ul的暂停和播放
      countdown_flg: false, //是否显示倒时器
      tooltip_list: [
        {
          name: "单拍子",
        },
